C------------------------------------------------------------------------------
C Version 22-October-2008                                       File: iextrae.f
C------------------------------------------------------------------------------
...
C dimensionada en el programa principal con la misma dimension que CLINEA.
        INTEGER FUNCTION IEXTRAE(CLINEA,N,ISTATUS,CBACKUP)
        IMPLICIT NONE
        CHARACTER*(*) CLINEA
        INTEGER N
        INTEGER ISTATUS
...
10      NEXT=INDEX(CBACKUP(L1:L2),' ')             !siguiente espacio en blanco
        IF(NEXT.EQ.0)THEN                     !ya no hay mas espacios en blanco
          IF(N.EQ.NCOL)THEN
            READ(CBACKUP(L1:L2),*,ERR=902) IEXTRAE
            ISTATUS=1
            RETURN
          ELSE
            GOTO 901
        ...
        END IF
        IF(N.EQ.NCOL)THEN
          READ(CBACKUP(L1:L1+NEXT-2),*,ERR=902) IEXTRAE
          ISTATUS=1
          RETURN
        END IF
...
C------------------------------------------------------------------------------
901     WRITE(*,*)
        WRITE(*,101) 'ERROR: unexpected end of row in IEXTRAE'
        WRITE(*,100) 'CLINEA: '
