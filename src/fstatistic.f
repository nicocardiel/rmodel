C------------------------------------------------------------------------------
C Version 22-October-2008                                    File: fstatistic.f
C------------------------------------------------------------------------------
...
C
        ...
        END
C******************************************************************************
C Calcula la abcisa a cuya derecha la distribucion normal tipificada cubre un 
C area (nivel de significacion) alpha
...
C
        ...
        END
C******************************************************************************
C Calcula la abcisa a cuya derecha la distribucion chi-cuadrado con N
C grados de libertad cubre un area (nivel de significacion) alpha
C
C Nota: me temo que esta funcion no funciona demasiado bien cuando N es muy
C pequen~o o muy grande
C
...
11    CONTINUE
      STOP 'FATAL ERROR: PAUSE, A too large, ITMAX too small'
...
C******************************************************************************
      ...
      IF(X.LT.0..OR.X.GT.1.) 
     + STOP 'FATAL ERROR: PAUSE, bad argument X in BETAI'
...
11    CONTINUE
      STOP 'FATAL ERROR: PAUSE, A or B too big, or ITMAX too small'
1     BETACF=AZ
      RETURN
      END
C******************************************************************************
...
C------------------------------------------------------------------------------
        SIGNIFICACION=ALPHA
        CALL DOWNHILL(1,X0,DX0,FUNK_IFNORTIP,1.0,0.5,2.0,1.E-7,
     >   XF,DXF,NEVAL,500)
        INVIERTE_FNORTIP=XF
        RETURN
        END
...
C------------------------------------------------------------------------------
C Valores iniciales par DOWNHILL
        X0=REAL(N)
        DX0=REAL(N)/10.
C------------------------------------------------------------------------------
        NDEG_OF_FREEDOM=N
        SIGNIFICACION=ALPHA
        CALL DOWNHILL(1,X0,DX0,FUNK_IFCHISQR,1.0,0.5,2.0,1.E-7,
     >   XF,DXF,NEVAL,500)
        INVIERTE_FCHISQR=XF
        RETURN
        END
